...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const topCompaniesResponse = await axios.get(`${VITE_JOB_API_URL}`);
  ...
  const handleFormSubmit = async (e, formType) => {
    ...
    try {
      ...
      if (formType === 'topCompanies') {
        const response = await axios.post(VITE_TOP_COMPANIES_API, {
        ...
        const response = await axios.post(`${VITE_JOB_API_URL}`, submissionData);
